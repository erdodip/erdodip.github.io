...
function write_out_the_result(result_list, box_id) {
  if (box_id == 1) {
    ...
    // Fix sort by value
    document.querySelector("#result_info_box_1").innerHTML =
      "Sort: " +
      result_list.sort(function (a, b) {
        return a - b
      })
  } else {
    document.querySelector("#result_info_box_2").innerHTML =
      "Head: O=" + result_list.pop() + ", Tail: X=" + result_list.pop()
...
document.querySelector("#dice_type").addEventListener("change", () => {
  ...
  document.querySelector("#label_dice_type").innerHTML = is_coin_selected
    ? "O"
    : "Dice"
})
